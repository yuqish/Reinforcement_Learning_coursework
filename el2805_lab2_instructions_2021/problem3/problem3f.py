...
nn_actor = torch.load('neural-network-3-actor.pth')
nn_critic = torch.load('neural-network-3-critic.pth')
...
for i in range(len(y_array)):
    for j in range(len(omega_array)):
        ...
        state_tensor = torch.tensor([[0, y, 0, 0, omega, 0, 0, 0]],
                                    requires_grad=False,
                                    dtype=torch.float32)
        Q = nn_critic(state_tensor)
        Q_array[i, j] = Q.detach().numpy()[0][0]
        mu, sigma2 = nn_actor(state_tensor)
        mu = mu.detach().numpy()[0]
        action_angle_array[i, j] = mu[1]
